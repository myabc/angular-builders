...
| "Unset array options arrive as `undefined`"   | No -- Angular CLI fills them with `[]` for builders outside `@angular/build:*`, and `@angular/build` treats `[]` as user-provided (no coverage reporters run, Vitest config reporters dropped). `src/unit-test/index.ts` deletes every empty array option before delegating. (#2420) |
