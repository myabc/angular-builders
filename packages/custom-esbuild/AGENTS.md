...
| "Unset array options arrive as `undefined`"   | No -- Angular CLI fills them with `[]` for builders outside `@angular/build:*`, and `@angular/build` treats `[]` as user-provided (no coverage reporters run, Vitest config reporters dropped, `include: []` runs no tests). `src/unit-test/index.ts` deletes every empty array option that is absent from the raw `angular.json` options (`context.getTargetOptions(context.target)`) before delegating; an explicit `[]` written by the user is kept. (#2420) |
